...
class H5Dataset(Dataset):
    ...
    def load_sim_and_cosmology(self, cosmo_path, gal_path):
        ...
        """
        cosmo_file = h5py.File(cosmo_path, 'r')
        gal_file = h5py.File(gal_path, 'r')

        cosmo = torch.tensor(np.array(cosmo_file['/vis/'])[np.newaxis, ...], dtype=torch.complex64)
        gal = torch.tensor(np.array(gal_file['/vis/'])[np.newaxis, ...], dtype=torch.complex64)
