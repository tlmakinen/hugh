...
MODEL_DIR = configs["moment_2_params"]["model_dir"]
LOAD_DIR = configs["moment_2_params"]["load_dir"]
...
# LOAD THE FIRST MOMENT NETWORK
print("LOADING FIRST MOMENT NETWORK")
model_1 = UNet3d(BasicBlock, filters=FILTERS).to(device)
model_1.load_state_dict(torch.load(osp.join(MODEL_PATH, MODEL_NAME)))



# INITIALISE THE SECOND MOMENT NETWORK
print("LOADING SECOND MOMENT NETWORK")
...
def test(plot=False):
    ...
    for i,data in tqdm(enumerate(val_dataloader)):
        ...
        # modify for the second moment calculation
        preds = model(x)
        y = (model_1(x) - y)**2
...
save_obj(history, "moment_2_train_history")
